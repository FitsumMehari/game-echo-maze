...
import type { RadarSnapshot } from "@/core/types";
import type { ThemeId } from "@/systems/settings";

const PALETTES: Record<
  ThemeId,
  { bg: string; wall: string; floor: string; hot: string; key: string; exit: string; peer: string }
> = {
  abyss: {
    bg: "#061019",
    wall: "#6aa8ff",
...
export function drawRadar(canvas: HTMLCanvasElement, snap: RadarSnapshot, theme: ThemeId, assist: boolean): void {
  ...
  for (const rc of snap.cells) {
    ...
    if (rc.cell === Cell.Wall || rc.cell === Cell.WallAbsorb || rc.cell === Cell.WallDecoy || rc.cell === Cell.Door)
  ...
  if (snap.memoryExit) {
    ...
    drawMarker(ctx, snap.memoryExit.ix - ox + 0.5, snap.memoryExit.iz - oz + 0.5, cell, p.exit, "square");
    ctx.globalAlpha = 1;
  }
  if (snap.memoryKey && !snap.keys.length) {
    ctx.globalAlpha = 0.4;
    drawMarker(ctx, snap.memoryKey.ix - ox + 0.5, snap.memoryKey.iz - oz + 0.5, cell, p.key, "diamond");
    ctx.globalAlpha = 1;
  }
  snap.enemies.forEach((e) => {
    const chasing = e.state === "chase" || e.state === "search";
    drawWorldMarker(ctx, e.x, e.z, ox, oz, cell, p.hot, chasing ? "triangle" : "circle");
  });
...
function drawMarker(
  ...
  cell: number,
  color: string,
  kind: "circle" | "ring" | "square" | "diamond" | "triangle",
): void {
  const x = gx * cell;
  const y = gz * cell;
