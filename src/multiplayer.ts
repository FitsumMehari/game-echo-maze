import type { RemotePeerState } from "./gameTypes";
...
export class MultiplayerClient {
  ...
  get status(): MultiplayerStatus {
    return { mode: this.mode, connected: this.mode !== "offline", label: this.statusLabel(), peers: this.livePeers().length };
  }
  ...
  connectWebSocket(url: string): void {
    ...
    this.socket.addEventListener("close", () => { this.mode = "offline"; this.emitChange("Relay disconnected"); });
  ...
  private receive(input: unknown): void {
    ...
    try { msg = typeof input === "string" ? JSON.parse(input) : input as WireMessage; } catch { return; }
