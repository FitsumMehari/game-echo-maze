...
import { CELL_SIZE, WALL_HEIGHT } from "@/core/constants";
import { Cell, type CellType, type ParsedLevel } from "@/world/level";
...
function makeBoxAt(ix: number, iz: number, floorY: number, cell: CellType): THREE.BufferGeometry | null {
...
export function buildWorldMeshes(
  ...
  for (let iz = 0; iz < height; iz++) {
    for (let ix = 0; ix < width; ix++) {
      const c = grid[iz]![ix]!;
      const doorIsOpen = c === Cell.Door && doorOpen;
      const isWalk =
        c === Cell.Floor ||
        c === Cell.Hazard ||
        c === Cell.Switch ||
        ...
        doorIsOpen;
      if (isWalk) {
        parts.push(makeFloorTile(ix, iz, doorIsOpen ? Cell.Floor : c));
      }

      if (c === Cell.Wall || c === Cell.WallAbsorb || c === Cell.WallDecoy || (c === Cell.Door && !doorOpen)) {
